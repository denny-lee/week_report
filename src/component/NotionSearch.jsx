...
const NotionSearch = React.createClass({
  	getInitialState() {
        return {
            itemList: []
        };
  	...
  	submitHandler(e) {
      	...
      	this.props.form.validateFields((err, values) => {
          	...
          	const respData = requestData(config.searchNotion, param, 'POST');
          	if (respData) {
  	...
  	render() {
	    ...
	    const items = itemList.map((k, i) => {
	    	...
	    	return <Card title={k.label} key={i}>
	    	...
	    	<div>
		      	<Form onSubmit={this.submitHandler} form={this.props.form}>
			        <Row type="flex" justify="center">
			            <Col span={8}>
			                <FormItem>
			                    <Input {...tagProp} placeholder="input tag or label hear." />
			                </FormItem>
			            </Col>
...
const NotionSearchForm = Form.create()(NotionSearch);
